        ...
        for edgeObj in edgeObjList:
            ...
            if source.title == "_start_":
                ...
                if dest not in addedNodes:
                    if dest.visible == 1:
                        node = {"id": dest.title, "count": dest.count, "height": defaultNodeHeight, "fill": nodeColor, "shape": initialNodeShape ,"normal": {"stroke": initialNodeStroke}, "correctness": dest.correctness, "fixedValue": dest.fixedValue, "linkedSolution": dest.linkedSolution, "visible": dest.visible, "modifiedCorrectness": 0}
                    ...
                    if dest.visible == 1:
                        ...
                        nodeList[pos] = {"id": dest.title, "count": dest.count, "height": defaultNodeHeight, "fill": nodeColor, "shape": initialNodeShape , "normal": {"stroke": initialNodeStroke}, "correctness": dest.correctness, "linkedSolution": dest.linkedSolution, "fixedValue": dest.fixedValue, "visible": dest.visible, "modifiedCorrectness": 0}
                
            elif dest.title == "_end_":
                nodeColor = getNodeColor(source)
                if source not in addedNodes:
                    if source.visible == 1:
                        node = {"id": source.title, "count": source.count, "height": defaultNodeHeight, "shape": finalNodeShape ,"fill": nodeColor, "normal": {"stroke": finalNodeStroke}, "correctness": source.correctness, "linkedSolution": source.linkedSolution, "fixedValue": source.fixedValue, "visible": source.visible, "modifiedCorrectness": 0}
                ...
                if source not in addedNodes:
                    if  source.visible == 1:
                        nodeColor = getNodeColor(source)
                        node = {"id": source.title, "count": source.count, "height": defaultNodeHeight, "fill": nodeColor, "correctness": source.correctness, "linkedSolution": source.linkedSolution, "fixedValue": source.fixedValue, "visible": source.visible, "modifiedCorrectness": 0}
                        if source.nodePositionX != -1 and source.nodePositionY != -1:
                            node["x"] = source.nodePositionX
                            node["y"] = source.nodePositionY
                            fixedPos = True
                ...
                if dest not in addedNodes:
                    if dest.visible == 1:
                        nodeColor = getNodeColor(dest)
                        node = {"id": dest.title, "count": dest.count, "height": defaultNodeHeight, "fill": nodeColor, "correctness": dest.correctness, "linkedSolution": dest.linkedSolution, "fixedValue": dest.fixedValue, "visible": dest.visible, "modifiedCorrectness": 0}
                        if dest.nodePositionX != -1 and dest.nodePositionY != -1:
                            node["x"] = dest.nodePositionX
                            node["y"] = dest.nodePositionY
                            fixedPos = True
